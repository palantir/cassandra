...
class Shell(cmd.Cmd):
    ...
    def print_static_result(self, column_names, rows, table_meta):
        ...
        column_names = column_names or list(table_meta.columns.keys())
        formatted_names = [self.myformat_colname(name, table_meta) for name in column_names]
        if not rows:
            # print header only
            self.print_formatted_result(formatted_names, None)
            return
        formatted_values = [list(map(self.myformat_value, [row[column] for column in column_names])) for row in rows]
