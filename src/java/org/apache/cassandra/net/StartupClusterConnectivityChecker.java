...
public class StartupClusterConnectivityChecker
{
    ...
    private final Predicate<InetAddress> gossipStatus;

    public StartupClusterConnectivityChecker(int targetPercent, int timeoutSecs, Predicate<InetAddress> gossipStatus)
    {
        if (targetPercent < 0)
        {
            targetPercent = 0;
    ...
    public void execute(Set<InetAddress> peers)
    {
        ...
        peers = peers.stream()
                     .filter(peer -> !peer.equals(FBUtilities.getBroadcastAddress()))
    ...
    State checkStatus(Set<InetAddress> peers, AtomicInteger connectedCount, final long startNanos, boolean beyondExpiration, final int completedRounds)
    {
     ...
     */
    private AtomicInteger sendPingMessages(Set<InetAddress> peers)
    {
