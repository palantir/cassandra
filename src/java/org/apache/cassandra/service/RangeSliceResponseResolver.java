...
public class RangeSliceResponseResolver implements IResponseResolver<RangeSliceReply, Iterable<Row>> {
    ...
    // Note: this would deserialize the response a 2nd time if getData was called
    // first.
    ...
    public Iterable<Row> resolve() {
        ...
        // for each row, compute the combination of all different versions seen, and
        // repair incomplete versions
    ...
    private class Reducer extends MergeIterator.Reducer<Pair<Row, InetAddress>, Row> {
        ...
        protected Row getReduced() {
            ...
            if (versions.size() < sources.size()) {
                // add placeholder rows for sources that didn't have any data, so
                // maybeScheduleRepairs sees them
