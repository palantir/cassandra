...
public class CassandraServer implements Cassandra.Iface
{
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        if (startSessionIfRequested())
        {
            ...
            Tracing.instance.begin("cell_cas", traceParameters);
        }
        else
        {
            logger.trace("cell_cas");
        }
        ...
        try
        {
            ...
            Iterable<ByteBuffer> names = Iterables.transform(column_updates,
                                                             new Function<ProposedColumnUpdate, ByteBuffer>() {
                public ByteBuffer apply(ProposedColumnUpdate proposedColumnUpdate)
                {
                    return proposedColumnUpdate.expected_column_name;
                }
            });
            ThriftValidation.validateColumnNames(metadata, new ColumnParent(column_family), names);

            Iterable<Column> proposedColumns = Iterables.transform(column_updates,
                                                             new Function<ProposedColumnUpdate, Column>() {
                public Column apply(ProposedColumnUpdate proposedColumnUpdate)
                {
                    return new Column()
                           .setName(proposedColumnUpdate.expected_column_name)
                           .setValue(proposedColumnUpdate.proposed_column_value);
                }
            });

            for (Column column : proposedColumns)
                ThriftValidation.validateColumnData(metadata, key, null, column);
