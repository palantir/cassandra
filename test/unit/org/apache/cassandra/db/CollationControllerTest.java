...
public class CollationControllerTest {
    ...
    public void getTopLevelColumnsSkipsSSTablesModifiedBeforeRowDelete() {
        ...
        // It should only iterate the last flushed sstable, since it probably contains
        // the most recent value for Column1
        ...
        // We will read "only" the last sstable in that case, but because the 2nd
        // sstable has a tombstone that is more
        // recent than the maxTimestamp of the very first sstable we flushed, we should
        // only read the 2 first sstables.
    ...
    public void ensureTombstonesAppliedAfterGCGS() {
        ...
        // use "realistic" query times since we'll compare these numbers to the local
        // deletion time of the tombstone
        QueryFilter filter;
        long queryAt = System.currentTimeMillis() + 1000;
        int gcBefore = cfs.gcBefore(queryAt);

        filter = QueryFilter.getNamesFilter(dk, cfs.name, FBUtilities.singleton(cellName, cfs.getComparator()),
                queryAt);
        CollationController controller = new CollationController(cfs, filter, gcBefore);
        assert ColumnFamilyStore.removeDeleted(controller.getTopLevelColumns(true, FilterExperiment.USE_OPTIMIZED),
                gcBefore) == null;
